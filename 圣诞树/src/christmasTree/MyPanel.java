...
public class MyPanel extends JPanel implements ActionListener{
	...
	MyPanel() {
		setLayout(null);
		ImageIcon icon = new ImageIcon("src/image/OFF.JPG");
	...
	public void paintComponent(Graphics g) {
		...
		y = 100 ;
		if(color) {
			ImageIcon image1 = new ImageIcon("src/image/2.JPG") ;
			g.drawImage(image1.getImage(), x-3, y-25, 28, 26, null) ;
		}
		else {
			ImageIcon image1 = new ImageIcon("src/image/1.JPG") ;
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == onOff) {		//按钮事件
			if(flag) {		//开
				ImageIcon icon = new ImageIcon("src/image/ON.JPG");
